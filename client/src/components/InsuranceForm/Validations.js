...
export const validationSchema = Yup.object({
	...
	birthdate: Yup.date()
		...
		.min(
			validDate(80),
			`You are too old. The latest valid date is ${validDate(80)}`,
		)
		.max(
			validDate(18),
			`You are too young. The earliest valid date is ${validDate(18)}`,
		),
	income: Yup.number().min(25000).required('Income is required'),
});
